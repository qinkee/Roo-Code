...
export function copyPaths(copyPaths, srcDir, dstDir) {
...
export function copyWasms(srcDir, distDir) {
    const nodeModulesDir = path.join(srcDir, "node_modules");
    fs.mkdirSync(distDir, { recursive: true });
    // Tiktoken WASM file.
    fs.copyFileSync(path.join(nodeModulesDir, "tiktoken", "lite", "tiktoken_bg.wasm"), path.join(distDir, "tiktoken_bg.wasm"));
    ...
    fs.copyFileSync(path.join(nodeModulesDir, "tiktoken", "lite", "tiktoken_bg.wasm"), path.join(workersDir, "tiktoken_bg.wasm"));
    console.log(`[copyWasms] Copied tiktoken WASMs to ${workersDir}`);
    // Main tree-sitter WASM file.
    fs.copyFileSync(path.join(nodeModulesDir, "web-tree-sitter", "tree-sitter.wasm"), path.join(distDir, "tree-sitter.wasm"));
...
export function copyLocales(srcDir, distDir) {
...
export function setupLocaleWatcher(srcDir, distDir) {
    ...
    try {
        ...
        console.error(`Error setting up watcher for ${localesDir}:`, error instanceof Error ? error.message : "Unknown error");
    }
}
export function generatePackageJson({ packageJson: { contributes, ...packageJson }, overrideJson, substitution, }) {
    ...
    return {
        ...
        ...overrideJson,
        contributes: {
            viewsContainers: transformArrayRecord(viewsContainers, from, to, ["id"]),
            views: transformArrayRecord(views, from, to, ["id"]),
            commands: transformArray(commands, from, to, "command"),
            menus: transformArrayRecord(menus, from, to, ["command", "submenu", "when"]),
            submenus: transformArray(submenus, from, to, "id"),
            configuration: {
                title: configuration.title,
                properties: transformRecord(configuration.properties, from, to),
            },
        },
    };
}
