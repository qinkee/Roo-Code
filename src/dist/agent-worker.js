...
function checkResourceLimits() {
	...
	if (resourceUsage.fileOperations > resourceQuota.maxFileOperations) {
		violations.push(
			`File operations limit exceeded: ${resourceUsage.fileOperations} > ${resourceQuota.maxFileOperations}`,
		)
	}

	if (resourceUsage.networkRequests > resourceQuota.maxNetworkRequests) {
		violations.push(
			`Network requests limit exceeded: ${resourceUsage.networkRequests} > ${resourceQuota.maxNetworkRequests}`,
		)
	}
