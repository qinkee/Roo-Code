import { AgentExecutor, RequestContext, DefaultRequestHandler, InMemoryTaskStore, AgentCard, ExecutionEventBus, DefaultExecutionEventBusManager } from "@a2a-js/sdk/server"
...
export class A2AServer {
	 ...
	 */
	async startAgentServerWithData(agent: AgentConfig, port: number = 0): Promise<{
		port: number
		url: string
		agentCard: any
	}> {
		try {
			...
			// 创建A2A AgentCard
			const a2aAgentCard = this.createA2AAgentCard(agent, port)
			...
			// 生成智能体卡片（兼容格式）
			const agentCard = this.generateAgentCard(agent, url)
	 ...
	 * 启动指定智能体的A2A服务
	 */
	async startAgentServer(agentId: string, port: number = 0): Promise<{
		port: number
		url: string
		agentCard: any
	}> {
	 ...
	 * 创建符合A2A标准的AgentCard
	 */
	private createA2AAgentCard(agent: AgentConfig, port: number): AgentCard {
		...
		return {
			...
			skills: agent.tools?.filter(t => t.enabled).map(tool => ({
				id: tool.toolId,
				name: tool.toolId,
		...
		const actualPort = await new Promise<number>((resolve, reject) => {
			...
			server.listen(targetPort, '127.0.0.1', () => {
				...
				if (address && typeof address === 'object') {
					logger.info(`[A2AServer] HTTP server started on port ${address.port}`)
	...
	async stopAllServers(): Promise<void> {
		const stopPromises = Array.from(this.servers.keys()).map(agentId => 
			this.stopAgentServer(agentId)
		)
	 ...
	 * 生成智能体卡片
	 */
	private generateAgentCard(agent: AgentConfig, url: string): any {
		return {
			name: agent.name,
			description: agent.roleDescription,
	...
	private async getAgentByAnyUser(agentId: string): Promise<AgentConfig | null> {
		try {
			...
			logger.info(`[A2AServer] Available agents for user ${currentUserId}:`, allAgents.map(a => ({ id: a.id, name: a.name })))
	 ...
	 * 执行智能体逻辑
	 */
	private async executeAgentLogic(agent: AgentConfig, request: any): Promise<{
		success: boolean
		data?: any
		error?: string
		duration?: number
