...
/**
 * IM认证服务 - 负责Roo-Code独立登录获取terminal=100的token
 */
export class IMAuthService {
    ...
    /**
     * 执行独立登录，获取terminal=100的accessToken
     */
    async login(): Promise<string> {
        try {
            this.outputChannel.appendLine('[IMAuth] Starting independent login for Roo-Code (terminal=100)...');
            ...
            const loginResponse = await fetch('http://127.0.0.1:8888/login', {
                method: 'POST',
                headers: {
                    'Token-Key': tokenKey,
