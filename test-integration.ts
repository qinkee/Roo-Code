...
import { ZeroWidthEncoder, MentionHelper, MentionParams } from "./src/utils/zeroWidthEncoder"

// 测试用例定义
interface TestCase {
	name: string
	input: string
	expectedParams?: MentionParams
	shouldPass: boolean
}
...
class IntegrationTester {
	 ...
	 * 测试基础编解码功能
	 */
	private async testBasicEncoding() {
		...
		for (const testCase of testCases) {
			try {
				const encoded = ZeroWidthEncoder.encode(testCase.data)
	 ...
	 * 测试任务提及
	 */
	private async testTaskMentions() {
		console.log(colors.blue("\n📋 测试任务提及"))
		console.log("─".repeat(40))
		...
		const testCases: TestCase[] = [
			{
				name: "新任务（无ID）",
				input: MentionHelper.createTaskMention("创建用户系统"),
				expectedParams: {
					type: "task",
					name: "创建用户系统",
				},
			...
			{
				name: "继续任务（有ID）",
				input: MentionHelper.createTaskMention("修复登录问题", "task-abc123"),
				expectedParams: {
					type: "task",
					name: "修复登录问题",
					id: "task-abc123",
			...
			{
				name: "任务名称包含特殊字符",
				input: MentionHelper.createTaskMention("实现@功能&优化#性能", "task-special"),
				expectedParams: {
					type: "task",
					name: "实现@功能&优化#性能",
					id: "task-special",
		...
		for (const testCase of testCases) {
			const result = MentionHelper.parseMention(testCase.input)
	 ...
	 * 测试智能体提及
	 */
	private async testAgentMentions() {
		console.log(colors.blue("\n🤖 测试智能体提及"))
		console.log("─".repeat(40))
		...
		const testCases: TestCase[] = [
			{
				name: "基础智能体",
				input: MentionHelper.createAgentMention("代码审查", "code-review"),
				expectedParams: {
					type: "agent",
					name: "代码审查",
					modeId: "code-review",
			...
			{
				name: "架构师智能体",
				input: MentionHelper.createAgentMention("系统架构师", "architect-mode"),
				expectedParams: {
					type: "agent",
					name: "系统架构师",
	 ...
	 * 测试命令参数构建
	 */
	private async testCommandParameters() {
		console.log(colors.blue("\n⚙️ 测试VSCode命令参数构建"))
		console.log("─".repeat(40))
		...
		const scenarios = [
			{
				name: "新任务命令",
				mention: MentionHelper.createTaskMention("实现搜索功能"),
			...
			{
				name: "继续任务命令",
				mention: MentionHelper.createTaskMention("继续优化", "task-12345"),
				expectedCommand: "roo-code.executeTask",
				expectedParams: {
					content: "继续优化",
					taskId: "task-12345",
			...
			{
				name: "智能体命令",
				mention: MentionHelper.createAgentMention("测试专家", "test-mode"),
		...
		for (const scenario of scenarios) {
			const parsed = MentionHelper.parseMention(scenario.mention)
		...
		const edgeCases = [
			{
				name: "空文本解析",
				test: () => {
					const result = MentionHelper.parseMention("")
			...
			},
			{
				...
				test: () => {
					...
					const result = MentionHelper.parseMention(text)
			...
			},
			{
				name: "清理零宽字符",
				test: () => {
					const mention = MentionHelper.createTaskMention("清理测试", "clean-id")
					const cleaned = ZeroWidthEncoder.cleanText(mention)
			...
			},
			{
				name: "检测零宽参数",
				test: () => {
					const withParams = MentionHelper.createTaskMention("测试")
					const withoutParams = "普通文本"
					return (
						MentionHelper.hasZeroWidthParams(withParams) === true &&
						MentionHelper.hasZeroWidthParams(withoutParams) === false
					)
				},
			},
			{
				name: "多个编码提取",
				test: () => {
					const text1 = MentionHelper.createTaskMention("任务1", "id1")
					const text2 = MentionHelper.createAgentMention("智能体1", "mode1")
					const combined = `开始 ${text1} 中间文本 ${text2} 结束`
					const all = ZeroWidthEncoder.extractAllFromText(combined)
	 ...
	 * 验证参数匹配
	 */
	private validateParams(actual: any, expected: any): boolean {
